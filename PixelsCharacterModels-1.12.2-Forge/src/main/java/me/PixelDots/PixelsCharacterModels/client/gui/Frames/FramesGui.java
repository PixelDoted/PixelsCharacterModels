...
public class FramesGui extends HandlerGui
{
	...
	public void init() {
		...
		UpdateBTNs();
		textBox = new TextFieldWidget(fontRenderer, 10, 80, 50, 20, new StringTextComponent("name")); addTextField(textBox);
		super.init();
	}
	...
	public void UpdateBTNs() {
		...
		for (int i = 0; i < Main.frames.list.size(); i++) {
			int num = i;
			Button b = new Button(120 + 10 + (60*col), (15*((row + 1) + row) + 5), 50, 20, new StringTextComponent(Main.frames.list.get(i).id), (value) -> {
				Main.GuiSettings.SelectedFrameID = num;
				Minecraft.getInstance().displayGuiScreen(new FramesGui());
			});
			if (Main.GuiSettings.SelectedFrameID != -1) if (i == Main.GuiSettings.SelectedFrameID) b.field_230693_o_ = false;
			addButton( b );
			row++;
			if (row >= 11) {
				row = 0;
