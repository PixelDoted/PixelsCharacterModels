...
public class Utillities 
{
	...
	//Child's ModelRenderer
		...
		public static void removeChild(ModelRenderer model, int ID) {
			...
			} else if (ObfuscationReflectionHelper.getPrivateValue(ModelRenderer.class, model, "field_78805_m") instanceof ObjectList<?>) {
				((ObjectList<ModelRenderer>)ObfuscationReflectionHelper.getPrivateValue(ModelRenderer.class, model, "field_78805_m")).remove(ID);
			}
		}
		
		@SuppressWarnings("unchecked")
		public static ModelRenderer getChild(ModelRenderer model, int ID) {
			if (childsSize(model) != 0) return null;
			if (model instanceof PartModelRenderer) {
				return ((PartModelRenderer)model).childModels.get(ID);
			} else if (ObfuscationReflectionHelper.getPrivateValue(ModelRenderer.class, model, "field_78805_m") instanceof ObjectList<?>) {
				return ((ObjectList<ModelRenderer>)ObfuscationReflectionHelper.getPrivateValue(ModelRenderer.class, model, "field_78805_m")).get(ID);
			}
			return null;
		}
		...
		public static int childsSize(ModelRenderer model) {
			if (model instanceof PartModelRenderer) {
				...
				return ((ObjectList<ModelRenderer>)ObfuscationReflectionHelper.getPrivateValue(ModelRenderer.class, model, "field_78805_m")).size();
			}
			return -1;
		}
		...
		public static void clearChildren(ModelRenderer model) {
			if (model instanceof PartModelRenderer) {
				...
				((ObjectList<ModelRenderer>)ObfuscationReflectionHelper.getPrivateValue(ModelRenderer.class, model, "field_78805_m")).clear();
			}
		}
