...
public class SkinsCommand {
    ...
    private static Integer setPlayerSkinByName(PlayerEntity sourcePlayer, PlayerEntity targetPlayer, String playerName) {
    	if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Loading skin...")), sourcePlayer.getUniqueID());
        SharedPool.get().execute(() -> {
            if (!SkinProviderController.GetInstance().setPlayerSkinByName(targetPlayer.getGameProfile(), playerName, true)) {
            	if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Could not load the skin")), sourcePlayer.getUniqueID());
            } else {
            	if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Skin loaded successfully")), sourcePlayer.getUniqueID());
            }
            if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new StringTextComponent("you can disable loading chat messages in the PCM settings menu"), sourcePlayer.getUniqueID());
        });
        return 1;
    }
    ...
    private static Integer setPlayerCapeByName(PlayerEntity sourcePlayer, PlayerEntity targetPlayer, String playerName) {
    	if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Loading cape...")), sourcePlayer.getUniqueID());
        SharedPool.get().execute(() -> {
            if (!SkinProviderController.GetInstance().setPlayerCapeByName(targetPlayer.getGameProfile(), playerName, true)) {
            	if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Could not load the cape")), sourcePlayer.getUniqueID());
            } else {
            	if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Cape loaded successfully")), sourcePlayer.getUniqueID());
            }
        });
        return 1;
    }
    ...
    private static Integer cleanPlayerSkin(CommandSource source, Entity targetEntity) throws CommandSyntaxException {
        PlayerEntity sourcePlayer = (PlayerEntity) source.assertIsEntity();
        PlayerEntity targetPlayer = (PlayerEntity) targetEntity;
        if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Removing skin...")), sourcePlayer.getUniqueID());
        SharedPool.get().execute(() -> {
            SkinProviderController.GetInstance().cleanPlayerSkin(targetPlayer.getGameProfile());
            if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Skin removed successfully")), sourcePlayer.getUniqueID());
        });
        return 1;
    }
    ...
    private static Integer cleanPlayerCape(CommandSource source, Entity targetEntity) throws CommandSyntaxException {
        PlayerEntity sourcePlayer = (PlayerEntity) source.assertIsEntity();
        PlayerEntity targetPlayer = (PlayerEntity) targetEntity;
        if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Removing cape...")), sourcePlayer.getUniqueID());
        SharedPool.get().execute(() -> {
            SkinProviderController.GetInstance().cleanPlayerCape(targetPlayer.getGameProfile());
            if (Main.OtherSaveData.showLoadingDatainChat) sourcePlayer.sendMessage(new TranslationTextComponent("chat.type.announcement", ISSUER, new StringTextComponent("Cape removed successfully")), sourcePlayer.getUniqueID());
        });
        return 1;
    }
}