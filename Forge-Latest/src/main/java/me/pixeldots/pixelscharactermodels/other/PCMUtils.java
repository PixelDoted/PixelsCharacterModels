...
public class PCMUtils {
        ...
        if (mesh != null) {
            for (MeshReader.Point[] points : mesh.faces) {
                for (MeshReader.Point point : points) {
                    ...
                    Vec2 uv = mesh.uvs.get(point.uv-1);
                    Vector3f normal = mesh.normals.get(point.normal-1);

                    output += "vertex " + vertex.x() + " " + vertex.y() + " " + vertex.y() + " ";
    ...
    public static ResourceLocation getEntityTexture(LivingEntity entity) {
        return PCMClient.minecraft.getEntityRenderDispatcher().getRenderer(entity).getTextureLocation(entity);
    }
