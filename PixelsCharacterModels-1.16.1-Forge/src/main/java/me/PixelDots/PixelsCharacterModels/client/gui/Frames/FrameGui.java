...
public class FrameGui extends HandlerGui
{
	...
	public void init() {
		...
		UpdateBTNs();
		Time = new TextFieldWidget(fontRenderer, 310,30,50,20, "Time"); addTextField(Time);
	...
	public void UpdateBTNs() {
		...
		Linear = addButton(new Button(250,30,50,20, Translation.Linear, (value) -> {
			if (value.getMessage() == Translation.Linear) value.setMessage(Translation.Linear);//Translation.Curve
			else value.setMessage(Translation.Linear);
			if (Main.GuiSettings.SelectedFrameID != -1) {
				Main.frames.list.get(Main.GuiSettings.SelectedFrameID).Linear = value.getMessage() == Translation.Linear ? true : false;
		...
		Loop = addButton(new Button(370,30,50,20, Translation.FALSE, (value) -> {
			...
			if (Main.GuiSettings.SelectedFrameID != -1) {
				Main.frames.list.get(Main.GuiSettings.SelectedFrameID).Loop = value.getMessage() == Translation.FALSE ? false : true;
				Main.frames.SaveFrames(Main.GuiSettings.SelectedFrameID);
			}
		}) );
		FrameReset = addButton(new Button(425, 30, 50, 20, Translation.TRUE, (value) -> {
			...
			if (Main.GuiSettings.SelectedFrameID != -1) {
				Main.frames.list.get(Main.GuiSettings.SelectedFrameID).resetnextframe = value.getMessage() == Translation.FALSE ? false : true;
				Main.frames.SaveFrames(Main.GuiSettings.SelectedFrameID);
			}
		}));
		if (Main.GuiSettings.SelectedFrameID != -1)
			FrameReset.setMessage(String.valueOf( Main.frames.list.get(Main.GuiSettings.SelectedFrameID).resetnextframe ));
	...
	public void drawText(int clr) {
		...
		drawString(fontRenderer, TimeText, 310, 70, clr);
		drawString(fontRenderer, Translation.Loop, 370, 70, clr);
		drawString(fontRenderer, "Reset old frame on next frame", 425, 10, clr);
	}
