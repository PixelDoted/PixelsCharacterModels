...
public class SettingsGui extends HandlerGui {
	...
	public void init() {
		...
		AnimKey1 = new TextFieldWidget(fontRenderer, 60, 60, 50, 20, "1"); addTextField(AnimKey1);
		AnimKey2 = new TextFieldWidget(fontRenderer, 60, 80, 50, 20, "2"); addTextField(AnimKey2);
		AnimKey3 = new TextFieldWidget(fontRenderer, 60, 100, 50, 20, "3"); addTextField(AnimKey3);
		AnimKey4 = new TextFieldWidget(fontRenderer, 60, 120, 50, 20, "4"); addTextField(AnimKey4);
		AnimKey5 = new TextFieldWidget(fontRenderer, 60, 140, 50, 20, "5"); addTextField(AnimKey5);
		...
		/*if (Sliders.getMessage() == Translation.Sliders) {
			...
			URLLength = new TextFieldWidget(fontRenderer, 60, 190, 50, 20, "URLLength"); addTextField(URLLength);
		//}
	...
	public void UpdateBTNs() {
		...
		AnimFrame1 = addButton(new Button(120, 60, 50, 20, Translation.Animation, (value) -> {
			if (value.getMessage() == Translation.Animation) value.setMessage(Translation.Frame);
			else value.setMessage(Translation.Animation);
			Main.animations.Anim1isFrames = value.getMessage() == Translation.Animation ? false : true;
			SaveData();
		}));
		AnimFrame2 = addButton(new Button(120, 80, 50, 20, Translation.Animation, (value) -> {
			if (value.getMessage() == Translation.Animation) value.setMessage(Translation.Frame);
			else value.setMessage(Translation.Animation);
			Main.animations.Anim2isFrames = value.getMessage() == Translation.Animation ? false : true;
			SaveData();
		}));
		AnimFrame3 = addButton(new Button(120, 100, 50, 20, Translation.Animation, (value) -> {
			if (value.getMessage() == Translation.Animation) value.setMessage(Translation.Frame);
			else value.setMessage(Translation.Animation);
			Main.animations.Anim3isFrames = value.getMessage() == Translation.Animation ? false : true;
			SaveData();
		}));
		AnimFrame4 = addButton(new Button(120, 120, 50, 20, Translation.Animation, (value) -> {
			if (value.getMessage() == Translation.Animation) value.setMessage(Translation.Frame);
			else value.setMessage(Translation.Animation);
			Main.animations.Anim4isFrames = value.getMessage() == Translation.Animation ? false : true;
			SaveData();
		}));
		AnimFrame5 = addButton(new Button(120, 140, 50, 20, Translation.Animation, (value) -> {
			if (value.getMessage() == Translation.Animation) value.setMessage(Translation.Frame);
			else value.setMessage(Translation.Animation);
			Main.animations.Anim5isFrames = value.getMessage() == Translation.Animation ? false : true;
			SaveData();
		}));
		
		Sliders = addButton(new Button(60, 165, 50, 20, Translation.Sliders, (value) -> {
			if (value.getMessage() == Translation.Sliders) value.setMessage(Translation.TextFields);
			else value.setMessage(Translation.Sliders);
			if (value.getMessage() == Translation.Sliders) Main.OtherSaveData.sliders = true;
		...
		showNameTags = addButton(new Button(fontRenderer.getStringWidth("show NameTags") + 60, 215, 50, 20, Translation.TRUE, (value) -> {
			if (value.getMessage() == Translation.FALSE) value.setMessage(Translation.TRUE);
			else value.setMessage(Translation.FALSE);
			if (value.getMessage() == Translation.TRUE) Main.OtherSaveData.showNameTags = true;
		...
		showLoadingDatainChat = addButton(new Button(fontRenderer.getStringWidth("show Loading Data in Chat") + 60, 240, 50, 20, Translation.TRUE, (value) -> {
			...
			if (value.getMessage() == Translation.TRUE) Main.OtherSaveData.showLoadingDatainChat = true;
	...
	public void drawText(int clr) {
		drawString(fontRenderer, Translation.animkey1, 10, 67, clr);
		drawString(fontRenderer, Translation.animkey2, 10, 87, clr);
		drawString(fontRenderer, Translation.animkey3, 10, 107, clr);
		drawString(fontRenderer, Translation.animkey4, 10, 127, clr);
		drawString(fontRenderer, Translation.animkey5, 10, 147, clr);
		drawString(fontRenderer, Translation.URL + " " + Translation.Length, 10, 197, clr);
		drawString(fontRenderer, "show NameTags", 60, 215, clr);
		drawString(fontRenderer, "show Loading Data in Chat", 60, 240, clr);
	}
