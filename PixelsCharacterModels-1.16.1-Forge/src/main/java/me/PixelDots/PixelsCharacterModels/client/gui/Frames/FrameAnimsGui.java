	...
	public void UpdateBTNs() {
		...
		if (Main.GuiSettings.SelectedFrameID != -1) {
			...
			for (int i = 0; i < frame.animNames.size(); i++) {
				int num = i;
				Button b = new Button(120 + 10 + (60*col), (15*((row + 1) + row) + 5), 50, 20, frame.animNames.get(i), (value) -> {
				...
				if (Main.GuiSettings.SelectedAnimationID == Main.animations.getAnimIDfromName(frame.animNames.get(i))) b.active = false;
				addButton(b);
				row++;
				if (row >= 11) {
					col++;
