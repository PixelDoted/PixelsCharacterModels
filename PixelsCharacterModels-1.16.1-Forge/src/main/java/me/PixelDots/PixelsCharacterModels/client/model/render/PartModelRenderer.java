...
public class PartModelRenderer extends ModelRenderer 
{
	...
	public ObjectList<PartModelRenderer> childModels = new ObjectArrayList<>();
	...
	public void copyParentValues() {
		...
		this.rotateAngleX = anchor.X + (useAnimRot ? animRot.X : 0) + ParentRotAngle.X;
		this.rotateAngleY = anchor.Y + (useAnimRot ? animRot.Y : 0) + ParentRotAngle.Y;
		this.rotateAngleZ = anchor.Z + (useAnimRot ? animRot.Z : 0) + ParentRotAngle.Z;
		
		this.rotationPointX = (useAnimPoint ? animPoint.X : 0) + data.RotPointX + ParentRotPoint.X;
		this.rotationPointY = (useAnimPoint ? animPoint.Y : 0) + data.RotPointY + ParentRotPoint.Y + 22.5f;
		this.rotationPointZ = (useAnimPoint ? animPoint.Z : 0) + data.RotPointZ + ParentRotPoint.Z;
		...
		if (data != null) {
			if (data.Show) {
				...
				if (!(myData.toString() == data.toString())) {
					...
					matrixStackIn.scale(data.Scale, data.Scale, data.Scale);
					if (renderer != null) renderer.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, red, green, blue, alpha);
					//else super.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, red, green, blue, alpha);
					if (rendererWear != null)
						rendererWear.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, red, green, blue, alpha);
>>>>>>> Stashed changes
