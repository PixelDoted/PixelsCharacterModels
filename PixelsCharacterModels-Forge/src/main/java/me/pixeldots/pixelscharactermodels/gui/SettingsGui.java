...
public class SettingsGui extends EntityGuiHandler {
    ...
    public SettingsGui(LivingEntity _entity) {
        ...
        // TODO: version = FabricLoader.getInstance().getModContainer("pcm").get().getMetadata().getVersion().getFriendlyString();
    }
    ...
    public void init() {
        ...
        A.setResponder((s) -> { PCMMain.settings.animations[0] = s; }); A.setMessage(Component.literal(PCMMain.settings.animations[0]));
        EditBox B = addTextField(new EditBox(textRenderer, 5+60, 165, 170, 10, Component.literal("")));
        B.setResponder((s) -> { PCMMain.settings.animations[1] = s; }); B.setMessage(Component.literal(PCMMain.settings.animations[1]));
        EditBox C = addTextField(new EditBox(textRenderer, 5+60, 180, 170, 10, Component.literal(""))); 
        C.setResponder((s) -> { PCMMain.settings.animations[2] = s; }); C.setMessage(Component.literal(PCMMain.settings.animations[2]));
        EditBox D = addTextField(new EditBox(textRenderer, 5+60, 195, 170, 10, Component.literal(""))); 
        D.setResponder((s) -> { PCMMain.settings.animations[3] = s; }); D.setMessage(Component.literal(PCMMain.settings.animations[3]));
        EditBox E = addTextField(new EditBox(textRenderer, 5+60, 210, 170, 10, Component.literal(""))); 
        E.setResponder((s) -> { PCMMain.settings.animations[4] = s; }); E.setMessage(Component.literal(PCMMain.settings.animations[4]));
